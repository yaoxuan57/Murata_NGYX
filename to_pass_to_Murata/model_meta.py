...
def resolve_sensor_info(
    ...
    stem = Path(data_source_path).stem
    # Split outputs: .../AHU_2_9_Blower_DE_A_r0.6_0.2_0.2/train.csv -> parent folder name.
    if stem in ("train", "val", "test"):
        stem = Path(data_source_path).parent.name
    ...
    # Fuzzy: match mapping name contained in candidate or vice versa.
...
def build_model_name(sensor_name: str, version: str) -> str:
    slug = sensor_name.replace(" ", "_").replace("2-9", "2_9")
    slug = re.sub(r"[^\w\-]+", "_", slug)
    slug = re.sub(r"_+", "_", slug).strip("_")
    version = str(version).strip()
    if not version.startswith("v"):
        version = f"v{version}"
    return f"{slug}_{version}" if slug else f"transformer_{version}"
...
def build_model_meta_payload(
    *,
    ...
    payload: Dict[str, Any] = {
        "modelName": build_model_name(sensor_name, version),
        "modelType": "transformer_forecast",
        "sensorId": sensor["sensorId"],
        "sensorName": sensor_name,
        "machineId": sensor["machineId"],
        "trainedAt": (trained_at or datetime.now(timezone.utc)).strftime("%Y-%m-%dT%H:%M:%SZ"),
        "description": (
            f"Transformer forecasting for {best_config.get('value_column', 'target')} "
